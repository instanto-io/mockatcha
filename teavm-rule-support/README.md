...
`TConcurrentLinkedQueue` fixes that. It is backed by an `ArrayDeque`, which is
enough for a single-threaded runtime. The emulation also covers other code that
constructs a `Description` or uses the queue directly.
...
That test does not extend `StrictTest` or open a `MockatchaSession` itself.
